...
{
    private void InitStageRewardTable()
    {    
    }

    public void SetStageRewardData(string in_sheet_data)
    {
        // 클래스에 있는 변수들을 순서대로 저장한 배열
        FieldInfo[] fields = typeof(StageRewardData).GetFields(BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance);
        ...
        for (int row = 0; row < rows.Length; row++)
        {
            ...
            StageRewardData tableData = new StageRewardData();
            ...
            if (m_dic_stage_reward_data.ContainsKey(tableData.m_stage))
                m_dic_stage_reward_data[tableData.m_stage].Add(tableData);
            else
                m_dic_stage_reward_data.Add(tableData.m_stage, new List<StageRewardData>() { tableData });
        }
    }
}