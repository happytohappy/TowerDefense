...
public partial class TableManager
{
    ...
    public void SetStageInfoData(string in_sheet_data)
    {
        // 클래스에 있는 변수들을 순서대로 저장한 배열
        FieldInfo[] fields = typeof(StageInfoData).GetFields(BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance);

        string[] rows = in_sheet_data.Split('\n');
        for (int row = 0; row < rows.Length; row++)
        {
            var sheetData = rows[row].Split('\t');
            StageInfoData tableData = new StageInfoData();
            ...
            m_dic_stage_info_data.Add(tableData.m_stage, tableData);
        }
    }
}