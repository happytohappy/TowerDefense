...
public partial class TableManager
{
    ...
    public void SetMissionAchievementData(string in_sheet_data)
    {
        // 클래스에 있는 변수들을 순서대로 저장한 배열
        FieldInfo[] fields = typeof(MissionAchievementData).GetFields(BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance);
        ...
        for (int row = 0; row < rows.Length; row++)
        {
            ...
            MissionAchievementData tableData = new MissionAchievementData();
            ...
            if (m_dic_mission_achievement_data.ContainsKey(tableData.m_kind))
            {
                m_dic_mission_achievement_data[tableData.m_kind].Add(tableData.m_sequence, tableData);
            }
            else
            {
                var value = new Dictionary<int, MissionAchievementData>() { { tableData.m_sequence, tableData } };
                m_dic_mission_achievement_data.Add(tableData.m_kind, value);
            }
        }
    }
}