...
public partial class TableManager
{
    ...
    public void SetRecruitInfoData(string in_sheet_data)
    {
        // 클래스에 있는 변수들을 순서대로 저장한 배열
        FieldInfo[] fields = typeof(RecruitInfoData).GetFields(BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance);
        ...
        for (int row = 0; row < rows.Length; row++)
        {
            ...
            RecruitInfoData tableData = new RecruitInfoData();
            ...
            if (m_dic_recruit_info_data.ContainsKey(tableData.m_kind))
                m_dic_recruit_info_data[tableData.m_kind].Add(tableData);
            else
                m_dic_recruit_info_data.Add(tableData.m_kind, new List<RecruitInfoData>() { tableData });
        }
    }
}