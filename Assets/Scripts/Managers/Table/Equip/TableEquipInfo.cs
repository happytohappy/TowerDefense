...
{
    private void InitEquipInfoTable()
    {    
    }

    public void SetEquipInfoData(string in_sheet_data)
    {
        // 클래스에 있는 변수들을 순서대로 저장한 배열
        FieldInfo[] fields = typeof(EquipInfoData).GetFields(BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance);

        string[] rows = in_sheet_data.Split('\n');
        for (int row = 0; row < rows.Length; row++)
        {
            var sheetData = rows[row].Split('\t');
            EquipInfoData tableData = new EquipInfoData();
            ...
            m_dic_equip_info_data.Add(tableData.m_kind, tableData);
        }
    }
}